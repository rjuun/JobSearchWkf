...
| **C4** Skills section | `C4. Build and Manage the Skills Section.md` | **code** | The Keep rows' `requirement_skills` — the skills carried by the tailored bullets, for requirements that have matched evidence — grouped by the matched requirement's rank (Core → Important → Nice-to-Have) and deduped to the highest. No ranking pass of its own; only Nice-to-Have sheds if the section overruns |
