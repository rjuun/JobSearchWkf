...
| `requirement_tailoring` | requirement_id→, evidence_ref (the `ref_code`), original_text, **cv_position**, cv_bullet, cv_placement, my_skills[] (candidate's own vocabulary — C2's selection from skills_master/star_competences/star_attributes, validated against them; traceability, not a display field), requirement_skills[] (JD-language skills this row demonstrates — B2's at C2 time, overwritten by C3 with the bullet's own tag; this is what the CV Skills section prints), **approval_status** | ~27 rows — the **C2** bridge / human-in-the-loop |
