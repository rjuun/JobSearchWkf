...
| `job_leads` | external_id, title, company_id→, office_id→, source_url, posted_days, applicant_count, accepting_applications + liveness_checked_at (tri-state posting liveness; NULL = never checked), **status**, roadblocks jsonb, misalignments jsonb, jd_group_primary, jd_group_secondary, `skill_ratings jsonb` (17 A–Q), ats_system, **b6 dimension scores + overall + recommendation**, bullet_bank_version | ~140 leads — the hub |
...
| `requirement_tailoring` | requirement_id→, evidence_ref (the `ref_code`), original_text, **cv_position**, cv_bullet, cv_placement, my_skills[] (candidate's own vocabulary — C2's selection from skills_master/star_competences/star_attributes, validated against them; traceability, not a display field), requirement_skills[] (what the JD asks of this requirement — B2's extraction, snapshotted at C2 and never rewritten), cv_bullet_skills[] (what the tailored bullet displays — C3's bracketed tag; this is what the CV Skills section prints), **approval_status** | ~27 rows — the **C2** bridge / human-in-the-loop |
