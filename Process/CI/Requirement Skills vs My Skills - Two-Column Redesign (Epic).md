...
- **Q3** — Should "My Skills" sourcing formally widen to `star_competences` + `star_attributes`, not just `skills_master`? This changes what C4's query pulls from and is the one open question with real code impact — recommend deciding this one before Claude Code starts on the C4 rewrite.
  - **ANSWERED 2026-08-23 — yes, all three tables** (Reggie, on [[C5 Skills Selection Produces Unreadable Overflow]]): a Job Description does not distinguish a skill from a competence from an attribute, even though the profile tables do.
  - **This question blocked the C4 rewrite and was never answered before it shipped.** The build took a third option the design never listed: `mySkills: ev.skills` — a verbatim copy of the evidence node's own free-text graph tags (`star_actions.skills`, `responsibilities.skills`, `bullet_bank.tags`), which is neither `skills_master` nor the widened set. §2 of this note had specified My Skills as "drawn from `skills_master`, and (pending Q3) `star_competences` / `star_attributes`"; no code ever did that. Measured on the live profile 2026-08-23: 246 distinct tags across 104 evidence rows, 180 used exactly once, only 8 present in any curated table. C4's uncapped consistency rule then printed all of a Keep set's tags verbatim — 67 in one line for the Allianz Partners lead. Closed by [[C5 Skills Selection Produces Unreadable Overflow]], which implements the §2 design: C2 now *selects* My Skills from all three tables and the write path drops anything they don't contain.
  - One deviation from §2 worth recording: §2 also said "C4 stops recomputing skills from scratch — it builds the CV Skills List primarily from the Keep-gated rows' `my_skills`, and checks coverage against `requirement_skills`". That is now **inverted**, on Reggie's explicit decision: the CV Skills section prints `requirement_skills` from the Keep rows, and My Skills stays what C3 §B.5 always called it — the traceability chain, bullet-level provenance, not a display field. Rationale: nothing in the rendered experience section ever referenced a My Skills value, so the consistency rule was guarding a correspondence that did not exist.
